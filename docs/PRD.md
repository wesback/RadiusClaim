...
| `deploy-dapr-components-workload-identity.sh` | Backfill Dapr CRDs with workload identity or SP auth | ✅ Primary |
| `deploy-dapr-components.sh` | Legacy backfill (SP only) | ⚠️ Deprecated |
...
**[MEDIUM] Radius Validation Checklist Update for Workload Identity**  
`docs/radius-validation-checklist.md` troubleshooting sections still reference service-principal-with-secret auth patterns that have been superseded by workload identity. Prerequisites and post-deployment sections need updating to reflect the current auth model.
