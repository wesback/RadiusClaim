...
expenses.MapPost("/", async (HttpContext context, ExpenseSubmission submission, DaprClient daprClient, ILogger<Program> logger, CancellationToken cancellationToken) =>
{
    var traceId = context.Items[CorrelationIdContextKey] as string ?? "unknown";
    ...
    CancellationToken cancellationToken)
{
    ...
    catch (Exception ex) when (!cancellationToken.IsCancellationRequested)
    {
        logger.LogWarning(
            ex,
            "ETag conflict or transient error on expense record write, retrying. ExpenseId={ExpenseId}",
            record.ExpenseId);
    }
...
internal sealed record ExpenseApprovalActionRequest(
    /// <summary>
    /// Reason for rejection. If not provided, defaults to "Manual rejection by approver".
    /// </summary>
    string? Reason = null);
