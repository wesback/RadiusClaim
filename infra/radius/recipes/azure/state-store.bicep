...
// - Local administrator account (administratorLogin: 'pgadmin') is created with a deterministic
...
@description('Local administrator login name for the PostgreSQL server. Required by Azure when passwordAuth is Enabled. Dapr uses Entra auth; this account is used for bootstrap post-deployment configuration only.')
...
// Deterministic admin password — derived from the recipe context so it is stable
// across re-runs of the same deployment. Prefix/suffix ensure Azure password
// complexity rules are met (uppercase, lowercase, digit, symbol).
var postgresAdminPassword = 'P${uniqueString(context.resource.id, serverName)}#9a'
...
// ---------------------------------------------------------------------------
// PostgreSQL Flexible Server — dual authentication (Entra + password)
// ---------------------------------------------------------------------------
// The server accepts BOTH Microsoft Entra token auth (for workloads with
// managed identity) AND local password auth. Password auth is required
// because Azure ARM mandates administratorLogin/administratorLoginPassword
// when passwordAuth is Enabled, and Dapr's state.postgresql component uses
// the SCRAM handshake with an Azure AD token as the password (useAzureAD).
//
// authConfig:
//   activeDirectoryAuth: 'Enabled'  — Entra token logins accepted (used by Dapr useAzureAD)
//   passwordAuth: 'Enabled'         — Allows SCRAM auth path Dapr uses to submit AD tokens
//   tenantId                        — Only tokens from this tenant are trusted
//
// administratorLogin / administratorLoginPassword are required by Azure ARM on
// server creation when passwordAuth is Enabled. Bootstrap uses ARM API calls
// (not psql) for post-deploy config, so the admin password is not exposed
...
output resourceMetadata object = {
  ...
  location: location
  authStrategy: 'entra-with-password-fallback'  // passwordAuth Enabled required for Dapr SCRAM+AD-token path
  ...
  dapr: {
    ...
    metadata: {
      ...
      // for Dapr Actors. This recipe migrated FROM blob storage for exactly this reason.
      actorStateStore: 'true'
      // keyPrefix: none — all RadiusClaim services (expense-api, workflow-engine)
      // share state under the same key namespace. Without this, Dapr defaults to
      // 'appid' prefix, which scopes keys per app ID and breaks cross-service reads
      // (e.g., workflow-engine can't read records written by expense-api).
      keyPrefix: 'none'
    }
  }
}
