...
# the default namespace so Kubernetes can pull images from ghcr.io.
# Uses --dry-run=client | apply so the operation is idempotent.
# Skipped with a warning when GHCR_TOKEN or GHCR_USERNAME is empty.
ensure_ghcr_pull_secret() {
  section "GHCR image pull secret"

  if [ -z "$GHCR_TOKEN" ] || [ -z "$GHCR_USERNAME" ]; then
    log_warning "GHCR_TOKEN and/or GHCR_USERNAME are not set -- skipping pull secret creation."
    log_warning "Pods that pull from a private ghcr.io package will fail to start on a fresh cluster."
    log_warning "Re-run with --ghcr-token <token> or export GHCR_TOKEN=<token> (and GHCR_USERNAME=<user>) to create it."
    return 0
  fi
  ...
  # This secret is created in 'default' as a cluster-level baseline only.
  # bootstrap.sh is responsible for patching it into the app workload namespace
  # (e.g. radiusclaim-azure-radiusclaim) during app deployment via its own
  # patch-service-account step. If deploying manually without bootstrap.sh,
  # copy this secret into your target namespace to avoid image pull failures.
}
...
# Auto-populate GHCR credentials from the gh CLI when available, mirroring
# bootstrap.sh's pattern.  GHCR credentials are optional for prepare-cluster —
# the pull-secret step is skipped with a warning when they are absent.
if [ -z "${GHCR_TOKEN:-}" ] || [ -z "${GHCR_USERNAME:-}" ]; then
...
if [ "$CREATE_SPN" = true ]; then
  echo "Next command       : ./scripts/bootstrap.sh --resource-group ${RESOURCE_GROUP} --create-spn --setup-workload-identity --yes"
else
  echo "Next command       : ./scripts/bootstrap.sh --resource-group ${RESOURCE_GROUP} --setup-workload-identity --yes"
fi
