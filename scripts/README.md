...
- Authenticated to target registry:
  - For GHCR: `docker login ghcr.io` or use `gh auth login` (GitHub CLI) which auto-detects your username
...
1. **Authenticate to GHCR** (one-time per session):
   ```bash
   # Option 1: Use GitHub CLI (auto-detects your username)
...
**Example:**
```bash
# For GHCR with explicit auth
export GHCR_TOKEN="your_github_pat_with_write_packages_scope"

# If already authenticated via docker login (recommended)
...
1. **Pre-authenticated docker** (recommended):
   ```bash
   docker login ghcr.io  # Uses your GitHub CLI session or manual credentials
   # Then run publish script (username is auto-detected)
   ./scripts/publish-radius-recipes.sh ghcr.io/your-username/radiusclaim/recipes local
   ```

2. **Environment variables** (for automation when docker login isn't available):
   ```bash
   export GHCR_TOKEN="ghp_..."  # GitHub PAT with 'write:packages' scope
   export GHCR_USERNAME="your-github-username"
   ./scripts/publish-radius-recipes.sh ghcr.io/your-username/radiusclaim/recipes local
   ```
   Note: `GHCR_USERNAME` is auto-detected from `gh auth` by default; set explicitly only if needed.
...
- Radius Validation Checklist: [`docs/radius-validation-checklist.md`](../docs/radius-validation-checklist.md)
