...
# │                                                                     │
# │  This script is OBSOLETE as of Phase 2b (Bootstrap Simplification). │
# │                                                                     │
# │  What moved where:                                                 │
# │  • RBAC role assignments → Radius recipes (inline)                 │
# │  • Component CRD generation → Radius recipes (inline)              │
# │  • Managed identity creation → workload-identity.bicep             │
# │  • Federated credentials → workload-identity.bicep                 │
# │  • Resource discovery → Recipe outputs (resourceMetadata)          │
# │                                                                     │
# │  What remains necessary:                                           │
# │  • Service account annotation → annotate-service-accounts.sh       │
# │                                                                     │
# │  Use annotate-service-accounts.sh instead.                         │
# └─────────────────────────────────────────────────────────────────────┘

echo "ERROR: This script is deprecated and should not be used."
echo ""
echo "Phase 2b Migration Summary:"
echo "  - RBAC role assignments now handled by Radius recipes"
echo "  - Component CRDs now created by Radius recipes"
echo "  - Managed identity + federated credentials now in workload-identity.bicep"
echo ""
echo "For service account annotation, use:"
echo "  scripts/annotate-service-accounts.sh --namespace <ns> --client-id <id>"
echo ""
exit 1
...
echo "     kubectl logs -n $NAMESPACE deployment/expense-api -c daprd --tail=200 | grep 'Component loaded'"
echo "     # NOTE: Radius pods use label app.kubernetes.io/name (not app=). Use deployment/<name> for reliability."
echo ""
