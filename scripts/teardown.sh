...
Optional:
  ...
  --delete-and-recreate-rg          Delete the resource group and recreate it (empty, ready for bootstrap)
  --rg-location <location>          Azure region for resource group recreation (default: francecentral)
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --include-resource-group)
      ...
      shift
      ;;
    --delete-and-recreate-rg)
      DELETE_AND_RECREATE_RG=true
      INCLUDE_RESOURCE_GROUP=true
      shift
      ;;
    --rg-location)
...
# ---------------------------------------------------------------------------
[ -n "$RESOURCE_GROUP" ] || {
  log_error "--resource-group is required. No safe default exists — specify the Azure resource group to tear down."
...
delete_radius_resources() {
  ...
  if rad_workspace_exists && rad_group_exists; then
    ...
    run_cmd "$RAD_BIN" group delete "$GROUP_NAME" -w "$WORKSPACE_NAME" -y
...
purge_soft_deleted_keyvaults() {
  section "Soft-deleted Key Vaults"

  local subscription_id
  subscription_id="$(az account show --query id -o tsv 2>/dev/null || true)"

  if [ -z "$subscription_id" ]; then
    log_warning "Could not determine subscription ID — skipping soft-delete Key Vault purge"
    return 0
  fi

  # List all soft-deleted Key Vaults in the subscription
  local soft_deleted_vaults
  soft_deleted_vaults="$(az keyvault list-deleted --query "[].name" -o tsv 2>/dev/null || true)"

  if [ -z "$soft_deleted_vaults" ]; then
    log_info "No soft-deleted Key Vaults found"
    return 0
  fi

  log_info "Found soft-deleted Key Vault(s) — purging to prevent deployment conflicts:"
  while IFS= read -r vault_name; do
    [ -n "$vault_name" ] || continue
    log_info "  Purging soft-deleted vault: ${vault_name}"
    run_cmd az keyvault purge --name "$vault_name" --no-wait 2>/dev/null || log_warning "  Could not purge '${vault_name}' — may require manual intervention"
  done <<< "$soft_deleted_vaults"

  log_success "Soft-deleted Key Vault purge initiated"
}
...
recreate_resource_group() {
  section "Azure resource group recreation"

  log_info "Waiting for resource group '${RESOURCE_GROUP}' deletion to complete ..."
  local max_wait=600  # 10 minutes
  local waited=0
  while rg_exists && [ $waited -lt $max_wait ]; do
    sleep 10
    waited=$((waited + 10))
    if [ $((waited % 60)) -eq 0 ]; then
      log_info "Still waiting... ($((waited / 60)) minutes elapsed)"
    fi
  done

  if rg_exists; then
    log_warning "Resource group deletion did not complete within ${max_wait}s. It may still be deleting."
    log_warning "Wait for deletion to complete, then run: az group create --name '$RESOURCE_GROUP' --location '$RG_LOCATION'"
    return 0
  fi

  log_success "Resource group '${RESOURCE_GROUP}' deleted successfully"
  log_info "Creating fresh resource group '${RESOURCE_GROUP}' in '${RG_LOCATION}' ..."
  run_cmd az group create --name "$RESOURCE_GROUP" --location "$RG_LOCATION" --output none
  log_success "Resource group '${RESOURCE_GROUP}' recreated (empty, ready for bootstrap)"
}
...
echo "  Recreate RG:           ${DELETE_AND_RECREATE_RG}"
if [ "$DELETE_AND_RECREATE_RG" = true ]; then
...
# 6 — Purge soft-deleted Key Vaults (must happen before RG deletion to prevent ARM conflicts)
purge_soft_deleted_keyvaults

# 7 & 8 — Azure resources (individual or entire RG)
