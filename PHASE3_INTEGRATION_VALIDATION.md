...
>
> **Additional note:** State store has since migrated from Blob Storage (as described in this document) to PostgreSQL for ACID transaction support required by Dapr Actors. See `infra/radius/recipes/azure/state-store.bicep` for current implementation.
