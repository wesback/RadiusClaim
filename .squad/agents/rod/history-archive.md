...
### 2026-04-07: Sweden Central deployment (`radiusclaim-swe-rg` / `radiusclaim-swe-aks`)

**Deployment target:**
- Region: `swedencentral` — confirmed valid (Recommended tier, 3 AZ, paired to `swedensouth`)
- Resource group: `radiusclaim-swe-rg` (dedicated, not mixing with `radiusclaim-rg` / eastus)
- AKS cluster: `radiusclaim-swe-aks`
- OIDC issuer: `swedencentral.oic.prod-aks.azure.com`

**Prepare-cluster execution notes:**
- Dapr helm chart (`dapr.github.io/helm-charts`) timed out on retries 1–3 and retry 5. Retry 4 succeeded for the control plane; dashboard install failed. All Dapr system services (dapr-operator, dapr-sidecar-injector, dapr-sentry, dapr-placement-server, dapr-scheduler-server) were Running healthy after retry 4.
- Contour (Radius dependency) also timed out on Radius install retry 1. Retry 2 found the existing partial install and completed. Radius v0.56.0 operational.
- Both Dapr and Radius helm install retries are driven by flaky helm chart registry reachability — not Sweden Central-specific. Retry logic in prepare-cluster.sh handles this correctly.
- Bootstrap auto-detected the existing Radius SP credential (AZURE_CLIENT_ID + AZURE_TENANT_ID), resolving auth mode as `wi` (workload identity). No second SP was created by bootstrap.

**Critical issue found: PostgreSQL recipe missing "Allow Azure Services" firewall rule**
- Symptom: All 3 workload pods entered CrashLoopBackOff on the post-restart cycle. Dapr sidecar logged: `failed to ping the database: context deadline exceeded` for `statestore (state.postgresql/v2)`.
- Root cause: The `state-store.bicep` recipe deploys PostgreSQL with `publicNetworkAccess: Enabled` but the `allowAzureServices` parameter defaulted to `false`, so NO firewall rules were created. TCP connections from AKS pods to `pgstatea219ae.postgres.database.azure.com:5432` were dropped.
- Azure Key Vault and Service Bus connected fine (they allow by default via managed identity VNET trust); PostgreSQL does not.
- Fix: Added firewall rule `AllowAzureServices` (`0.0.0.0 → 0.0.0.0`) manually via `az postgres flexible-server firewall-rule create`. Pods recovered to 2/2 Running on next rollout.
- **Root fix needed**: The `state-store.bicep` recipe should default `allowAzureServices=true` OR the bootstrap should add this firewall rule post-deployment. See decisions inbox.

**Bootstrap sequence confirmed for Sweden Central:**
```bash
# Phase 1: Cluster prep (creates AKS, installs Dapr+Radius, registers SP)
./scripts/prepare-cluster.sh \
  --resource-group radiusclaim-swe-rg \
  --location swedencentral \
  --aks-cluster-name radiusclaim-swe-aks \
  --create-aks --create-spn --install-dapr --install-radius --yes

# Refresh context before phase 2 (rubber-duck finding)
az aks get-credentials --resource-group radiusclaim-swe-rg --name radiusclaim-swe-aks --overwrite-existing

# Phase 2: App bootstrap (workload identity, Radius env, app deploy)
./scripts/bootstrap.sh \
  --resource-group radiusclaim-swe-rg \
  --location swedencentral \
  --aks-cluster-name radiusclaim-swe-aks \
  --create-spn --setup-workload-identity --yes
```

**Cluster verification steps that worked:**
- `dapr status -k` → shows all 5 Dapr system services healthy
- `kubectl get pods -n radius-system` → 6/6 Radius pods Running
- `kubectl get pods -n radiusclaim-azure-radiusclaim` → all workloads 2/2 Running
- `kubectl get components -n radiusclaim-azure-radiusclaim` → 3 components (platform-secrets, pubsub, statestore)
- `kubectl logs deployment/expense-api -c daprd` → no errors, scheduler connected, actor dissemination complete

**Port-forward command for testing:**
```bash
kubectl port-forward -n radiusclaim-azure-radiusclaim svc/expense-api 8080:8080
```
Then: `curl http://localhost:8080/healthz` or `curl http://localhost:8080/`

### 2026-04-07: Workflow instance `609be63197f541a29953e5792e1bead6` — cross-sidecar race bug confirmed and fixed

**Symptom:** `ExpenseApprovalWorkflow` started and failed with `ORCHESTRATION_STATUS_FAILED` within ~17ms. "Waiting on runtime signal" and "notification event not emitted" — never reached the approval wait step.

**Diagnostic steps taken:**
1. Found the specific instance in Dapr sidecar logs: start + FAILED within the same second (11:39:59 UTC).
2. Checked app logs — found the exception stack trace:
   ```
   System.InvalidOperationException: Expense '06054d06b2ed4c3cae6609891021f80a' was not found in state store before approval.
   at WorkflowEngine.Activities.ApproveExpenseActivity.RunAsync (line 28)
   ```
3. Confirmed no Dapr sidecar crash, no OOM, no pub/sub or Service Bus issue — all Dapr components healthy, all pods 2/2 Running with 0 restarts.
4. Cross-referenced `decisions.md` — the fix for this exact race was documented (Billy's decision, dated 2026-04-01) but **never committed to the codebase**. The file had only one git commit: the initial commit.

**Root cause:** Cross-sidecar write-visibility race. expense-api persists the expense record and then immediately invokes workflow-engine. `ApproveExpenseActivity` reads the state store through the workflow-engine Dapr sidecar, which hasn't yet received the write from the expense-api sidecar. The activity threw immediately on null record, failing the entire workflow before it could reach `WaitForExternalEventAsync`.

**Fix applied:** `ApproveExpenseActivity.cs`
- Moved threshold/decision calculation BEFORE the state store read
- Changed null guard from `throw InvalidOperationException` to `log + return decision`
- The approval routing decision (auto-approve vs manual review) comes from `input.Amount`, which is authoritative in the workflow contract
- State store read still runs; when present, all correlation/state-transition validations still apply

**Tests updated:** `ApproveExpenseActivityTests.cs`
- Replaced `MissingStateRecord_ThrowsInvalidOperationException` with two tests:
  - `MissingStateRecord_UnderThreshold_ReturnsAutoApproveFromInput` — verifies correct routing and no `SaveStateAsync` called
  - `MissingStateRecord_AtThreshold_ReturnsManualReviewFromInput` — verifies threshold boundary holds even without the record

**Key insight:** This is a recurring pattern in this codebase. Any activity that reads from the state store immediately after the triggering service writes to it is susceptible to this race on AKS + multi-sidecar deployments. The fix pattern is: carry authoritative routing data in the workflow input contract; treat state store reads as best-effort for validation/transition, not as mandatory blocking reads.

**Dapr/Radius infrastructure was not at fault.** The components, sidecars, state store, pub/sub, and pod health were all normal. This was purely an application-level bug in the activity code.

## Learnings

### 2026-07-14: Security Review — Dapr/Radius Platform (infra/radius/ + apply-dapr-components-from-recipes.sh)

Full findings written to `.squad/decisions/inbox/rod-dapr-radius-security.md`. Key observations:

**State Store:**
- `keyPrefix: none` is an intentional cross-service design choice but there are no ACL scopes on the Kubernetes Component CRD to prevent future services from reading/overwriting the shared namespace. This is a latent risk that grows as more services are added.
- `passwordAuth: Enabled` is required for Dapr's SCRAM+AD-token path. The admin password is deterministic (`uniqueString(context.resource.id, serverName)`) — stable across redeployments, but computable from observable deployment artifacts. Recommend Key Vault storage or post-deploy rotation.
- `allowAzureServices: true` default creates the `0.0.0.0/0.0.0.0` firewall rule — all Azure-hosted IPs can reach PostgreSQL, not just the AKS cluster. Private endpoints exist in the recipe but are not the default, and `publicNetworkAccess` is never explicitly disabled even when private endpoint is enabled.
- No Dapr `Resiliency` policy → state store outage causes unbounded blocking, not graceful degradation.

**Pub/Sub:**
- Entra-only auth and TLS 1.2 are correctly configured. SAS is disabled at the namespace level.
- No dead-letter topic or consumer anywhere. Reimbursement notifications can be silently dropped on subscriber failure.
- Service Bus Standard SKU — no sessions, no FIFO ordering, no duplicate detection window for approval/reimbursement events.
- RBAC assignment removed from recipe Bicep and done in bootstrap.sh — partial-state risk if bootstrap is interrupted.

**Service Invocation:**
- Dapr mTLS is on by default (dapr-sentry Running confirmed). Not pinned via a `Configuration` CRD.
- No `Resiliency` resource deployed — no timeout, retry, or circuit breaker for any component interaction.
- workflow-engine → expense-api connection declared as `http://expense-api:8080` — acceptable only if all traffic routes through the Dapr sidecar (port 3500); direct HTTP bypasses mTLS.

**Radius Lifecycle:**
- Core `rad deploy` is idempotent. PostgreSQL naming is deterministic. Key pattern: stable across redeployments.
- Post-deploy steps (PostgreSQL firewall, Entra admin, Service Bus RBAC, Key Vault RBAC) are all outside Radius — if bootstrap is interrupted, partial state is indistinguishable from healthy from the control plane perspective.
- `randomNameSuffix` creates new resources on redeployment rather than updating existing ones — hit in Sweden Central (firewall not added, pods CrashLooped).

**Cross-Service Visibility:**
- `X-Correlation-ID` is present on HTTP paths (frontend → expense-api → workflow-engine) and in notification-svc logs.
- Gap: correlation ID propagation across the pub/sub boundary is not verified. If `PublishEventAsync` does not include it in message metadata, notification delivery cannot be correlated to the originating request.
- OpenTelemetry traces go to Jaeger (dev only) — no cloud backend configured for deployed environments.

**Failure Modes:**
- Actor state survives sidecar restart (external PostgreSQL). Confirmed in Sweden Central.
- No circuit breaker → PostgreSQL unreachable = unbounded blocking, liveness probe failure.
- No dead-letter consumer → Service Bus transient failure = silent notification loss for reimbursements.

**Top 5 risks to address before production (in priority order):**
1. Deploy Dapr `Resiliency` with circuit breaker and timeout for statestore and pubsub
2. Configure Service Bus dead-letter handling and a compensating consumer in notification-svc
3. Restrict PostgreSQL network access — explicit AKS IP rules or private endpoint with `publicNetworkAccess: Disabled` enforced post-deploy
4. Rotate/randomize PostgreSQL admin password post-deployment (not derivable from logs)
5. Add Dapr Component `scopes` to state store CRD to prevent unintended cross-service access
## 2026-04-11 — Orchestrated Parallel Execution

Spawned by Scribe for concurrent work on:
- Workflow-engine authentication + input validation fixes
