...
## Learnings — Application Security Review (2026-03-27)

- **`RequireAuthorization()` without policy/scope checks is incomplete.** Both approve/reject endpoints authenticate the caller but do not enforce role claims, scopes, or ownership. Any valid Entra ID token bearer can approve any expense, including their own.
- **Workflow-engine has zero auth on any endpoint.** `POST /workflows/{instanceId}/decide` is completely open. Security relies entirely on Dapr service mesh network isolation — if that boundary is misconfigured, any network-reachable caller can flip a pending approval decision.
- **No upper bounds on `Amount`, `Description`, or `EmployeeId`.** Missing max-value and max-length constraints allow oversized submissions that flow through state, workflow, and pub/sub payloads without rejection.
- **`WorkflowStatusResponse.FailureDetails`** forwards `WorkflowState.FailureDetails.ToString()` directly to API callers. Can include internal exception messages or stack traces. Frontend escapes it (no XSS), but the content leakage is real.
- **Global expense-index is unscoped** — GET /expenses returns all expenses to any unauthenticated caller. Intentional for this sample, but a real deployment needs per-user scoping.
- **CorrelationId is client-controlled and used as the Dapr workflow instanceId.** A caller that knows a target expense's correlationId can probe workflow state endpoints without owning the expense.
- **Frontend XSS risk is well-controlled.** `escapeHtml()` is applied consistently across all data-to-innerHTML paths including description, employeeId, expenseId, failureDetails. Clean pattern worth preserving.
- **No hardcoded secrets in source.** All auth config sourced from appsettings + env vars; Dapr component names used instead of connection strings throughout. Clean baseline.
