...
- **Teardown script orchestration:** AKS clusters must be deleted before generic Azure resources (via `az resource delete --ids`). AKS managed resource groups (MC_*) are part of the cluster object lifecycle; deleting the AKS cluster resource properly cascades to node pools and managed dependencies. Generic resource deletion does not handle this dependency chain correctly. Pattern applied: guard with `aks_cluster_exists()`, use `--no-wait` for consistency with other script operations, and call before `delete_azure_resources()` in the main execution block.
...
## Learnings — Security Review (2026-07)

### Context
Billy requested a comprehensive security review covering architecture, threat model, secrets, authentication, data flow, and platform-level Dapr/Radius concerns.

### Key Architecture Findings
- Trust zones are well-structured: public → expense-api → Dapr mTLS → workflow-engine / notification-svc → Entra workload identity → Azure backing services. The Radius/Dapr boundary is clean.
- No hardcoded credentials anywhere in app or Bicep code. All sensitive config is externalized via env vars, Kubernetes secrets, or Radius recipe outputs.
- Entra-only auth pivot (PostgreSQL, Service Bus, Key Vault) is complete and correct. This is the strongest part of the security posture.

### Security Gaps Identified
1. **Unauthenticated GET /expenses and GET /expenses/{id}** — full financial records (EmployeeId, Amount, Description, RejectionReason) exposed to anonymous callers. High severity for a reference sample. Billy.
2. **workflow-engine endpoints have no RequireAuthorization** — `/workflows/start`, `/workflows/{id}/decide` can be called directly within the cluster without a token. Medium severity; may be acceptable as internal-only if explicitly documented. Billy.
3. **Azure Dapr components missing `scopes`** — generated manifests for statestore, pubsub, platform-secrets have no app-id scoping, unlike the correctly-scoped local Redis components. Medium severity. Graham.
4. **Deterministic PostgreSQL admin password** — derived from `uniqueString(context.resource.id, serverName)` which is ARM-visible. Password unused at runtime (Dapr uses Entra tokens) but exists on a publicly reachable server when `allowAzureServices=true`. Medium. Graham.
5. **`allowAzureServices: true` is the dev default** — 0.0.0.0 magic rule on PostgreSQL. Documented but not prominently warned in the walkthrough. Low. Eddie + Graham.
6. **PII in notification logs** — EmailTransport logs recipient, expenseId, correlationId at INFO. Fine for a stub but teaches a pattern of logging PII. Low. Billy.

### Scope Boundary Calls
- No network policies between services is a teaching gap but NOT a blocker for the reference sample; Dapr mTLS covers the primary in-cluster concern.
- HTTPS redirection absent from app code — handled at ingress level. Not flagging as an app-code concern.
- The notification transport being a stub is by design; no security concern.

### Verdict
**NEEDS WORK** — no critical infrastructure vulnerabilities, but unauthenticated reads of financial data and missing Dapr component scoping need addressing before this can be called a security-positive reference sample. Priority sequence: Billy (GET auth), Graham (scopes + password), Eddie (docs).

### Decision Document
Written to: `.squad/decisions/inbox/daisy-security-review.md`

## 2026-04-11 — Orchestrated Parallel Execution

Spawned by Scribe for concurrent work on:
- Fix VM SKU availability in germanywestcentral
