...
## 2026-06-11: Security Test Scenario Design (Billy request)

**Task:** Design a security test scenario matrix covering all six categories: Auth/Authz, Input Validation, Workflow/State, Data Leakage, Failure Modes, and Security Regression. No implementation — design only.

### Key Findings

**What is already covered (automated):**
- JWT authentication on approve/reject endpoints (`OAuth2AuthenticationTests`, 9 tests)
- Basic input validation — missing fields, non-positive amounts (`ExpenseApiValidationTests`)
- Activity idempotency — already-approved/already-reimbursed states (`ApproveExpenseActivityTests`)
- Pub/sub contract schema alignment (`NotificationContractTests`)
- Pagination boundary conditions (`ExpensePaginationTests`)

**Critical gaps discovered:**
1. **Employee self-approval is not blocked** — `POST /expenses/{id}/approve` requires a valid JWT but does NOT verify the token identity differs from the expense submitter's EmployeeId. Any authenticated user can approve their own expense.
2. **workflow-engine and notification-svc have no authentication** — both services are Dapr-internal, but there is no defense-in-depth auth layer. A pod with network access can call `/workflows/{instanceId}/decide` without any token.
3. **GET /expenses/ returns ALL users' expenses** — no EmployeeId filter based on JWT identity. Any caller can enumerate all expense records.
4. **No upper-bound validation on Amount or string field lengths** — Amount accepts decimal.MaxValue; string fields accept arbitrarily long input.
5. **ExpenseId injection risk** — ExpenseId becomes a Dapr state store key prefix (`expense:{expenseId}`). A crafted value like `expense-index` could collide with the index key.
6. **JWT audience validation not regression-tested** — `ValidateAudience = true` is set but if flipped to false, no CI test catches it.
7. **No duplicate-submission HTTP-level test** — internal dedup logic exists but the API HTTP response for duplicate ExpenseId is untested.
8. **No workflow-engine-down recovery test** — expense persists but workflow never starts; this silent failure path is untested.

### Learnings
- Self-approval gap is the highest-priority finding — it is a real authz logic gap, not just a missing test.
- The fire-and-forget workflow start pattern means a failed workflow start is silent from the client — intentional, but needs an observable failure mode test.
- Auth is JWT-only on expense-api; the other two services rely on Dapr network isolation as their security boundary. For a demo/reference app this is acceptable but should be noted in the threat model.
- Pagination parameter validation exists in unit tests but not as HTTP-level integration tests.

## 2026-04-11 — Orchestrated Parallel Execution

Spawned by Scribe for concurrent work on:
- Anticipatory test cases for approval flow
