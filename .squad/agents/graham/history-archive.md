...
## App-Modeling .gitignore Update

**Date:** 2026-05-05  
**Task:** Small repo hygiene — ignore app-modeling skill directory  

**Work:**
- Added `/.github/skills/app-modeling/` to root `.gitignore`
- Narrowest correct pattern covering full skill payload
- No changes to skill files themselves
- Decision recorded in `.squad/decisions.md`

**Context:** Isolates skill-specific build/runtime artifacts from core repository tracking.

---

## Portable sample review — contract drift (2026-05-05)

**Finding:** A Dapr + Radius sample stops being teachable as soon as the app model, bootstrap scripts,
tests, and README describe different platform contracts.

**Concrete drift found in RadiusClaim:**
- `infra/radius/environments/local.bicep` advertises local OCI recipes under `.../recipes/local`,
  but `scripts/publish-radius-recipes.sh` only publishes Azure recipes.
- `README.md` and `scripts/bootstrap.sh` assume a public `Applications.Core/gateways` resource,
  but `infra/radius/app.bicep` currently declares no gateway.
- `workflow-engine` expects `APP_API_TOKEN` in non-development environments, but `app.bicep`
  does not wire that secret or config into the workload.

**Takeaway:** For reference samples, treat the "platform contract surface" as one artifact:
model + environment + bootstrap + validation + docs must move together. If one layer changes
(recipe names, namespaces, ingress, auth, or secret flow), update every teaching surface in the
same change or the sample will create false confidence.
