...
Attempted to clean up stale Dapr components using:
- `rad resource delete Applications.Dapr/secretStores platform-secrets` — hung indefinitely
- `kubectl delete component ...` — succeeded, but Radius DB still had references

## What Was Tried

1. Direct `kubectl delete` of Dapr Component CRDs
   - ✅ Removes the Kubernetes resource
   - ❌ Doesn't clean Radius control plane state

2. `rad resource delete` command
   - ❌ Hung indefinitely on resources in "Updating" state

3. Control-plane restart
   - ⚠️ Partial success — some resources changed state, but orphaned references remain
...
Direct `kubectl delete` only clears Tier 1. The `rad resource delete` command is supposed to handle both, but hangs when resources are in Failed/Updating states.

## Potential Workarounds (Not Yet Tested)

1. **Full Radius reinstall:**
   ```bash
   rad uninstall kubernetes
   rad install kubernetes
   ```
   This would clear the control plane state, but requires re-creating the workspace and environment.

2. **Database access (if available):**
   If direct Radius database access is available, orphaned references could be purged directly. Contact Radius team for guidance on database schema.

3. **Radius control plane downgrade:**
   If a specific control plane version is known to handle orphaned resource cleanup better, downgrade to that version.
