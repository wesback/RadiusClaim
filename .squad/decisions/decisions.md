...
## 5. GHCR Auto-Detection Documentation Alignment
...
### Decision
Documentation updated to reflect Rod's GHCR_USERNAME auto-detection fix in `prepare-cluster.sh`. Manual export and hardcoded 'wesback' references removed; all examples now use `gh api user --jq .login` inline or rely on pre-authenticated docker login.

### Rationale
Scripts with auto-detection should lead documentation with the detection method (lowest friction). Manual export becomes optional fallback for CI/CD environments where `gh` isn't available.

### Key Principle
When deployment scripts auto-detect environment variables, documentation must:
1. Lead with auto-detection method
2. Show inline usage over separate export steps
3. Note manual export as optional fallback

### Files Updated
- `docs/radius-validation-checklist.md` — template placeholder + inline detection
- `docs/end-to-end-setup-walkthrough.md` — inline detection, removed manual export
- `scripts/README.md` — reordered authentication options (pre-auth docker first), marked env vars optional

### No Regressions
Scripts support both auto-detection and manual export paths (unchanged).
